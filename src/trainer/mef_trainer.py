    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                # Training step
                under_exp_img = batch["under"].to(self.device)
                over_exp_img = batch["over"].to(self.device)

                # Predict the noise residual
                fusion_pred, flow_net = self.model(under_exp_img, over_exp_img)
                ...
                loss_output = self.loss(
                    ...
                    under_exp_img,  # B, 5, C, H, W
                    over_exp_img,  # B, 5, C, H, W
                    flow_net,
                )
