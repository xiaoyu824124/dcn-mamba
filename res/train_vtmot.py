...
def main() -> None:
    ...
    parser.add_argument("--crop-hw", type=int, nargs=2, metavar=("HEIGHT", "WIDTH"), default=None,
                        help="override the configured common crop; use 480 640 for full-frame A4000 training")
    ...
    parser.add_argument("--init", type=Path, default=None,
                        help="optional compatible synthetic checkpoint; does not resume optimiser state")
    ...
    print(f"device={device} run={args.run} steps={steps} train={len(train_dataset)} eval={len(eval_dataset)} crop={crop_hw} batch={batch_size}")
    ...
    for step in range(start_step + 1, steps + 1):
        ...
        if step == 1 or step % int(train_config.log_every) == 0:
            print("step={step:5d} loss={train_loss:.5f} train_epe={train_epe:.3f}".format(**record))
        if step % int(train_config.validation_every) == 0 or step == steps:
            ...
            print("  eval epe={epe_px:.3f} zero={zero_flow_epe_px:.3f} ratio={relative_epe:.3f}".format(**validation))
